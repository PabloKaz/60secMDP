...
public class PrimeraPersonaController : MonoBehaviour
{
    public static PrimeraPersonaController instanciaCompartida;
   ...
   public float mouseSensitivity= 5f;
    ...
    private void Awake()
    {
        ...
        //Seteamos variables de agacharse
        escalaAgachado = originalScale;
        escalaAgachado.y = originalScale.y * .75f;
    }
    ...
    {   if (GameManager.instanciaCompartida.gameState == GameState.enLaPartida)
        {
            ...
            if (cameraCanMove)
            {
                yaw = transform.localEulerAngles.y + Input.GetAxis("Mouse X") * ViewOpciones.instanciaCompartida.sensibilidadDelMouse ;

                if (!invertCamera)
                {
                    pitch -= ViewOpciones.instanciaCompartida.sensibilidadDelMouse * Input.GetAxis("Mouse Y");
                }
                else
                {
                    // Inverted Y
            ...
            if (enableJump && Input.GetKeyDown(jumpKey) && isGrounded &&!isCrouched)
            {
                Jump();
            }
    ...
    private void Crouch()
    {
        ...
        /*if (agachado)
        {
            ...
            transform.localScale = Vector3.Lerp(transform.localScale, escalaAgachado, 0.1f);
            walkSpeed /= speedReduction;
            isCrouched = false;
        }
        // Crouches player down to set height
    ...
    void PausarYDespausar()
    {
        ...
        {
            if (Input.GetKeyDown(KeyCode.Escape)) GameManager.instanciaCompartida.StartGame();
        }
...
// Custom Editor
#if UNITY_EDITOR
    [CustomEditor(typeof(PrimeraPersonaController)), InitializeOnLoadAttribute]
    public class FirstPersonControllerEditor : Editor
    {
    PrimeraPersonaController fpc;
    SerializedObject SerFPC;

    private void OnEnable()
    {
        fpc = (PrimeraPersonaController)target;
